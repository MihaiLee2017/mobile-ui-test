/**
 * 参考地址 https://github.com/litehelpers/Cordova-sqlite-storage
 * @desc sqlite 组件封装
 * @author zhaolun.he@meicloud.com
 */
...
const sqlite = {
  ...
  deleteTabel (tableName) {
    return new Promise((resolve, reject) => {
      ...
      this.db.transaction((tx) => {
        ...
        tx.executeSql(`DROP TABLE IF EXISTS ${tableName}`, (ts, rs) => {
          console.log('删除成功', rs)
          resolve(rs)
        }, (ts, error) => {
          resolve(error)
  ...
  insertData (tableName, table) {
    return new Promise((resolve, reject) => {
      ...
      let fieldName = headers.join(',')
      let valuesStr = headers.fill('?').join(',')
      this.db.transaction((tx) => {
        console.log('插入SQL', `INSERT INTO ${tableName} (${fieldName}) VALUES (${valuesStr})`, values)
        tx.executeSql(`INSERT INTO ${tableName} (${fieldName}) VALUES (${valuesStr})`, values, (ts, rs) => {
          console.log('插入成功', rs)
          resolve(rs)
        }, (ts, error) => {
  ...
  deleteData (tableName, where, param) {
    return new Promise((resolve, reject) => {
      ...
      this.db.transaction((tx) => {
        ...
        tx.executeSql(`DELETE FROM ${tableName} WHERE ${where}`, param, (ts, rs) => {
  ...
  updateData (tableName, table, where, param) {
    return new Promise((resolve, reject) => {
      ...
      let conditon = values.concat(param)
      let setParam = ''
      ...
      this.db.transaction((tx) => {
        ...
        tx.executeSql(`UPDATE ${tableName} SET ${setParam} WHERE ${where}`, conditon, (ts, rs) => {
          console.log('更新成功', rs)
          resolve(rs)
        }, (ts, error) => {
  ...
  selectData (tableName, where, param, rows) {
    return new Promise((resolve, reject) => {
      ...
      this.db.transaction((tx) => {
        ...
        tx.executeSql(`SELECT ${row} FROM ${tableName} WHERE ${where}`, param, (ts, rs) => {
          let dataList = []
          if (rs.rows.length > 0) {
